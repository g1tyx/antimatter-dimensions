...
export default {
  ...
  computed: {
    ...
    boughtTooltip() {
      if (this.isCapped) return `无名氏限制最多购买${format(1)}个第8反物质维度`;
      if (this.isContinuumActive) return "continuum* 生产所有的反物质维度";
      return `购买了${this.bought}次`;
    },
    costUnit() {
      return `第${this.tier - 2}反物质维度`;
    },
    buttonPrefix() {
      if (!this.isUnlocked) return "已锁定";
      if (this.isCapped) return "被无名氏击破";
