...
function updateTickSpeed() {
  ...
  if (exp > 1) document.getElementById("tickSpeedAmount").textContent = '每轮速度: ' + player.tickspeed.toFixed(0);
  else {
      document.getElementById("tickSpeedAmount").textContent = '每轮速度: ' + player.tickspeed.times(new Decimal(100).dividedBy(Decimal.pow(10, exp))).toFixed(0) + ' / ' + shorten(new Decimal(100).dividedBy(Decimal.pow(10, exp)));
  }
}